...
import { UpdateProducerController } from "@/interfaces/controllers";
import CustomError, { ErrorCodes } from "@/domain/entity/error";

jest.mock("@/application/usecases", () => {
  return {
    UpdateProducer: jest.fn().mockImplementation(() => ({
      execute: jest.fn(),
    })),
  };
});
...
describe("UpdateProducerController", () => {
  let createProducerController: UpdateProducerController;
  let mockUsecase: jest.Mocked<UpdateProducer>;
  let mockInputValidator: jest.Mocked<InputValidator>;

  beforeEach(() => {
    mockUsecase = new (require("@/application/usecases").UpdateProducer)();

    createProducerController = new UpdateProducerController();
