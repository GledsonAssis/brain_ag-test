...
import { DeleteProducerController } from "@/interfaces/controllers";
import CustomError, { ErrorCodes } from "@/domain/entity/error";

jest.mock("@/application/usecases", () => {
  return {
    DeleteProducer: jest.fn().mockImplementation(() => ({
      execute: jest.fn(),
    })),
  };
});
...
describe("DeleteProducerController", () => {
  let createProducerController: DeleteProducerController;
  let mockUsecase: jest.Mocked<DeleteProducer>;
  let mockInputValidator: jest.Mocked<InputValidator>;

  beforeEach(() => {
    mockUsecase = new (require("@/application/usecases").DeleteProducer)();

    createProducerController = new DeleteProducerController();
